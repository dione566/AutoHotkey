...
SearchRadius  := 50
AimbotActive  := False
GuiVisible    := True ; <--- Nova variável para controlar a visibilidade da GUI
...
; F6 Oculta ou Mostra a GUI (Bolinha indicadora)
F6::
    GuiVisible := !GuiVisible ; Inverte o estado de visibilidade
    if (GuiVisible) {
        ; Se o script estiver suspenso, mostra cinza. Se não, mostra a cor do estado atual
        if (A_IsSuspended)
            Gui, Color, 888888
