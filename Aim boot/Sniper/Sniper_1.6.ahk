...
; -------------------------------------------------------------------------------------
; --- HOTKEY: WheelDown (Macro de Troca Rápida) ---
; -------------------------------------------------------------------------------------

WheelUp::
    ; Desativa o AimBot Sniper se estiver ativo
    if (MacroActive) {
    ...
    ; Alterna o estado do Macro de Troca Rápida
    QuickSwapMacroActive := True

    if (QuickSwapMacroActive) {
        ToolTip, ✅ Troca Rápida LIGADO, A_ScreenWidth/2, A_ScreenHeight/2 -50
        SoundBeep, 800, 150
    } else {
        ToolTip, 🚫 Troca Rápida DESLIGADO, A_ScreenWidth/2, A_ScreenHeight/2 -50
        SoundBeep, 400, 150
    }

    GlobalPaused := False ; Garante que a pausa global esteja desativada (se necessário)
    SetTimer, RemoveToolTip, -250
    return

; -------------------------------------------------------------------------------------
; --- HOTKEY: WheelUp (AimBot Sniper - Sequência de Q) ---
; -------------------------------------------------------------------------------------
...
; ---------------------------------------------------------------------
; --- HOTKEY: F8 (Executar Mira_02.ahk) ---
; ---------------------------------------------------------------------
F10::
    ; O comando Run executa o script na Área de Trabalho
    Run, %A_Desktop%\Mira_02.ahk
    
    ; Exibe uma confirmação visual
    ToolTip, ➕ Mira_02.ahk INICIADO, A_ScreenWidth/2, A_ScreenHeight/2 -50
    SoundBeep, 600, 150
    SetTimer, RemoveToolTip, -250
    return
...
; [NOVA FUNÇÃO]
; --- FUNÇÃO DE VERIFICAÇÃO CIRCULAR (Fórmula da Distância Euclidiana) ---
IsPixelInCircle(px, py, centerX, centerY, radius) {
    ; Distância ao quadrado = (x - x_centro)² + (y - y_centro)²
    DistanceSquared := (px - centerX) ** 2 + (py - centerY) ** 2
...
RecalculateSearchArea:
    ...
    ; Define a área retangular que circunscreve o círculo
...
; -------------------------------------------------------------------------------------
; --- LOOP 1: AimBot Padrão (Busca Circular Implementada) ---
; -------------------------------------------------------------------------------------

ColorAimbotLoop:
    IfWinActive, %GameWinTitle%
    {
        ; 1. Busca a cor na área quadrada (é o máximo que o PixelSearch faz)
        PixelSearch, FoundX, FoundY, SearchAreaX1, SearchAreaY1, SearchAreaX2, SearchAreaY2, ColorToFind, %Variation%, Fast RGB
        
        ; [MODIFICADO PARA BUSCA CIRCULAR]
        ; 2. Filtra o resultado: Somente se a cor for encontrada E estiver DENTRO do círculo
        ColorFound := False
        if (ErrorLevel = 0) {
            if (IsPixelInCircle(FoundX, FoundY, Coordenada_X_Tiro_Recalculada, Coordenada_Y_Tiro_Recalculada, SearchRadius)) {
                ColorFound := True
            }
        }
        
        ; 3. Lógica de Pressionar e Soltar o Botão
...
; -------------------------------------------------------------------------------------
; --- LOOP 2: Macro de Tiro + Q (AimBot Sniper) (Busca Circular Implementada) ---
; -------------------------------------------------------------------------------------
...
MacroShotLoop:
    IfWinActive, %GameWinTitle%
    {
        ...
        ; 3. Lógica da Sequência de Macro
        if (ColorFound) 
        {
...
; -------------------------------------------------------------------------------------
; --- MACRO: Troca Rápida de Armas (Condicional) ---
; -------------------------------------------------------------------------------------
