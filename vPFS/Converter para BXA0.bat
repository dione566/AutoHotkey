...
rem Nome da nova pasta que será criada para salvar os arquivos criptografados
set "NOME_PASTA=Hide"
...
rem Cria a nova pasta se ela ainda não existir
...
for /f "delims=" %%f in ('dir /b /a-d') do (
    ...
    if /i "!ext!"==".mp4" (
        ...
        echo Ocultando: "%%f" -> "%NOME_PASTA%\!nome_original!.bXA0"
        
        rem Executa a gravação dos 32 bytes de trava salvando dentro da nova pasta
        powershell -ExecutionPolicy Bypass -File temp_inject.ps1 -inFile "%%f" -outFile "%NOME_PASTA%\!nome_original!.bXA0"
        
        rem [OPCIONAL] Descomente a linha abaixo (remova o 'rem') se quiser deletar o MP4 original após gerar o bXA0
        rem del "%%f"
    )
)
...
echo.
echo ======================================================
echo  CONCLUÍDO! Os arquivos .bXA0 foram gerados em "%NOME_PASTA%".
