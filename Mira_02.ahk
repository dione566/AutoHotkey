...
; Status inicial: Começa com a mira de Cruz visível <--- ALTERADO
MiraAtual := "Cross" ; Pode ser "Dot" ou "Cross" <--- ALTERADO
ToggleVisibilidade := true ; Falso = Escondido, Verdadeiro = Visível <--- ALTERADO
...
; Exibe a GUI da Cruz e armazena o Handle (visível por padrão) <--- ALTERADO
Gui, 2: Show, x%CrossGui_X% y%CrossGui_Y% w%CrossGui_W% h%CrossGui_H% NoActivate, CrossGUI <--- 'Hide' REMOVIDO
...
; Esconde o Ponto no início, já que a mira CRUZ é a inicial <--- ALTERADO
Gui, 1: Hide ; <--- ADICIONADO para garantir que o Dot não apareça
Return 
...
{
    MiraAtual := "Dot"
    Gui, 2: Hide ; Esconde a Cruz <--- ALTERADO
    If ToggleVisibilidade ; Só mostra se F9 estiver ativo
        Gui, 1: Show ; Mostra o Ponto <--- ALTERADO
}
Else
{
    MiraAtual := "Cross"
    Gui, 1: Hide ; Esconde o Ponto <--- ALTERADO
    If ToggleVisibilidade ; Só mostra se F9 estiver ativo
        Gui, 2: Show ; Mostra a Cruz <--- ALTERADO
}
Return
